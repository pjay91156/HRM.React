...
import { Layers3, AlignLeft } from "lucide-react";
import { type Floor, type FloorFormData } from "../../models/Floor";
import Loader from "../common/Loader";

const emptyFormValues: FloorFormData = {
    name: "",
    description: ""
};
...
interface AddFloorModalProps {
    ...
    usedFloorNames: string[];
}

const AddFloorModal: React.FC<AddFloorModalProps> = ({
    isOpen,
    onClose,
    onSubmit: onSubmitFloor,
    loading = false,
    editingFloor = null,
    usedFloorNames
}) => {
    const isEditMode = !!editingFloor;
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <Layers3 size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-600" />
                            <input
                                id="name"
                                {...register("name", {
                                    validate: (value) => {
                                        const trimmed = value.trim();
                                        if (!trimmed) return "Floor name is required and cannot contain only spaces.";
                                        if (trimmed.length > 100) return "Floor name cannot exceed 100 characters.";
                                        if (usedFloorNames.includes(trimmed.toLowerCase())) {
                   ...
                   min-w-[110px] flex items-center justify-center"
                        >
                            {loading ? <Loader size="sm" /> : isEditMode ? "Update Floor" : "Save Floor"}
                        </button>
                    </div>
                </form>
            </div>
