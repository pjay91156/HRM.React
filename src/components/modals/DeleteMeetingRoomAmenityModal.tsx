...
interface DeleteMeetingRoomAmenityModalProps {
    isOpen: boolean;
    onClose: () => void;
    onConfirm: () => void;
    amenityName: string;
...
const DeleteMeetingRoomAmenityModal: React.FC<DeleteMeetingRoomAmenityModalProps> = ({
    isOpen,
    onClose,
    onConfirm,
    amenityName,
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-[1px]">
            <div className="w-full max-w-md rounded-xl bg-white dark:bg-slate-900 p-6 shadow-xl border border-gray-100 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center gap-3 text-red-600 mb-4">
                    ...
                    <h3 className="text-lg font-bold text-gray-900 dark:text-slate-100">Remove Amenity</h3>
                </div>

                {/* Content */}
                <div className="space-y-2 mb-6">
                    <p className="text-sm text-gray-600 dark:text-slate-400">
                        Are you sure you want to remove <span className="font-semibold text-gray-900 dark:text-slate-100">{amenityName}</span>?
                    </p>
                    <p className="text-xs text-gray-400 dark:text-slate-500">
                        This amenity will be deactivated and removed from any rooms it's assigned to. It will not be permanently deleted.
                    </p>
                </div>
                ...
                <div className="flex items-center justify-end gap-3">
                    ...
                    >
                        {loading ? <Loader size="sm" /> : "Delete Amenity"}
                    </button>
                </div>
            </div>
        </div>
...
export default DeleteMeetingRoomAmenityModal;
