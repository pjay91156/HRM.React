...
import AddMeetingRoomAmenityModal from '../components/modals/AddMeetingRoomAmenityModal';
import DeleteMeetingRoomAmenityModal from '../components/modals/DeleteMeetingRoomAmenityModal';
import Loader from "../components/common/Loader";

const MeetingRoomAmenityTable: React.FC = () => {
    ...
    const [editingAmenity, setEditingAmenity] = useState<MeetingRoomAmenity | null>(null);

    const [isDeleteModalOpen, setIsDeleteModalOpen] = useState<boolean>(false);
    const [amenityToDelete, setAmenityToDelete] = useState<MeetingRoomAmenity | null>(null);
    ...
    const loadAmenities = async () => {
        try {
            setLoading(true);
            const response = await meetingRoomAmenityService.getAmenities();
    ...
    const usedAmenityNames = useMemo(
        () =>
            amenities
                .filter((amenity) => amenity.id !== editingAmenity?.id)
                .map((amenity) => amenity.name.toLowerCase()),
        [amenities, editingAmenity]
    );
    ...
    const openEditModal = (amenity: MeetingRoomAmenity) => {
    ...
    const handleSubmitAmenity = async (data: MeetingRoomAmenityFormData) => {
        try {
            setSaving(true);

            if (editingAmenity) {
                await meetingRoomAmenityService.updateAmenity(editingAmenity.id, data);
            } else {
                await meetingRoomAmenityService.addAmenity(data);
            }
    ...
    const handleDeleteClick = (amenity: MeetingRoomAmenity) => {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            setDeleting(true);
            await meetingRoomAmenityService.deleteAmenity(amenityToDelete.id);
    ...
    return (
        <div className="space-y-6 max-w-[1600px] mx-auto">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Amenities</h1>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Manage the amenities available in your meeting rooms</p>
                </div>
                <button
            ...
            <div className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                <th className="sticky top-0 z-30 py-3.5 px-6 font-semibold bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-700">Amenity Name</th>
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {filteredAmenities.map(amenity => (
                                <tr key={amenity.id} className="group hover:bg-slate-50/70 transition-colors duration-150">
                                    <td className="py-4 px-6">
                                        <div className="font-medium text-slate-900 dark:text-slate-100 text-sm">{amenity.name}</div>
                                    </td>
                                    <td className="py-4 px-6">
                                        <div className="text-sm text-slate-500 dark:text-slate-400">{amenity.description || "—"}</div>
                                    </td>
                ...
                {!loading && filteredAmenities.length === 0 && (
                    <div className="p-16 text-center">
                        ...
                        <h3 className="text-sm font-medium text-slate-900 dark:text-slate-100">No amenities found</h3>
                        <p className="text-xs text-slate-400 dark:text-slate-500 mt-1 max-w-xs mx-auto">
                            Try adjusting your search terms, or add a new amenity to get started.
                        </p>
                    </div>
                )}
            </div>

            <AddMeetingRoomAmenityModal
                isOpen={isModalOpen}
                onClose={closeModal}
...
export default MeetingRoomAmenityTable;
