...
const formatDate = (value: string) => {
  ...
  const filteredCycles = useMemo(() => {
    return MOCK_CYCLES.filter(cycle => {
      ...
      const matchesStatus = statusFilter === "All" || cycle.status === Number(statusFilter);
      const matchesStartDate = !startDate || cycle.start >= startDate;
      const matchesEndDate = !endDate || cycle.end <= endDate;
      return matchesSearch && matchesStatus && matchesStartDate && matchesEndDate;
    });
  }, [search, statusFilter,search, startDate, endDate]);
  ...
  return (
    ...
    <button
        ...
        className="w-full h-11 flex items-center gap-2 rounded-xl
                   border border-slate-200 dark:border-slate-700
                   bg-slate-50 dark:bg-slate-800
                   px-4 text-sm shadow-sm text-left"
    >
        <CalendarDays
            size={18}
    ...
    <div className="w-full sm:w-56">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredCycles.map((cycle) => (
          <div key={cycle.id} className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start mb-2">
              <h3 className="font-bold text-xl dark:text-white">{cycle.name}</h3>
              <span className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 ${cycle.status === PerformanceCycleStatus.Active ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-600'}`}>
...
{expandedCard === cycle.id && (
    ...
    <div className="mb-6 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-900 p-4 animate-fade-in">
