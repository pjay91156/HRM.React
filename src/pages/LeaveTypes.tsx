    ...
    return (
        <div className="space-y-6 max-w-[1600px] mx-auto">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Leave Types</h1>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Manage and configure your leave policies</p>
                </div>
                <button 
            ...
            <div className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {filteredLeaveTypes.map(lt => (
                                <tr key={lt.id} className="group hover:bg-slate-50/70 transition-colors duration-150">
                                    <td className="py-4 px-6 font-medium text-slate-900 dark:text-slate-100 text-sm">{lt.leaveName}</td>
                                    <td className="py-4 px-4 text-sm font-mono text-slate-500 dark:text-slate-400">{lt.leaveCode}</td>
                                    <td className="py-4 px-4 text-sm text-slate-600 dark:text-slate-400">{lt.description}</td>
                                    <td className="py-4 px-4 text-sm text-slate-900 dark:text-slate-100">{lt.defaultDays}</td>
                ...
                {!loading && filteredLeaveTypes.length === 0 && (
                    <div className="p-16 text-center">
                        ...
                        <h3 className="text-sm font-medium text-slate-900 dark:text-slate-100">No leave types found</h3>
